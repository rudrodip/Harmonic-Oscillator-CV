...
class VideoThread(QThread):
    ...
    def run(self, save_data=False):
        while self._run_flag:
            ...
            contoursOutput = self.processor.get_contours(frame, mask)

            if self.display_option == "Image Contours":
                frame = contoursOutput["imageContours"]

            contours = contoursOutput["contours"]
            ...
            if contours:
                ...
                if cx > self.params['xmax']: self.params['xmax'] = cx
                if cx < self.params['xmin']: self.params['xmin'] = cx
            ...
            if self.draw_params and self.params:
                cv2.line(frame, (int(self.params['xmin']), int(fitted_b)), (int(self.params['xmax']), int(fitted_b)), (255, 255, 0), 2)
                cv2.circle(frame, (cx, int(fitted_b)), 5, (0, 0, 0), -1)

                cv2.line(frame, (int(fitted_a), int(fitted_b)), (cx, cy), (0, 255, 0), 2)
    ...
    def update_hsv_range(self, hmin, hmax, smin, smax, vmin, vmax):
        ...
        self.processor.hsvVals = hsvVals
